...
import { MockERC20, MockERC20__factory, ProviderController, ProviderController__factory } from "../types";
...
const setup = deployments.createFixture(async (hre) => {
  ...
  await deployments.fixture(["provider-controller"]);
...
describe("ProviderController test", () => {
  ...
  it("register provider", async () => {
    ...
    fee = 1_000_000_000_000n; // 1000 gwei
  ...
  it("can't register more than 200", async () => {
    for (let i = 1; i < 200; i++) {
    ...
    const registerKey = solidityKeccak256(["string"], [`Provider 200`]);
    const fee = 1_000_000_000_000;
    await expect(providerController.connect(alice).registerProvider(registerKey, fee)).to.revertedWith(
      "ProviderController: provider count reached max",
    );
  });
  ...
  it("withdraw provider earning", async () => {
    advanceTimeAndBlock(60 * 24 * 60 * 60); // 2 months
    ...
    expect(subscriberData.balance).eq(10_000_000_000_000n); // 12000 - 2000 = 10000 (wei)
  });
  ...
  it("test view functions", async () => {
    expect(await providerController.getProviderEarning(4)).to.eq(2_000_000_000_000n); // 2 months with 1000 gwei
    expect(await providerController.getSubscriberRemaining(2)).to.eq(6_000_000_000_000n); // 2 months with 1000 gwei for three providers
  });
});
