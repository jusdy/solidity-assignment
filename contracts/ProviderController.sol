 ...
 * @dev The contract you are working on is a Provider-Subscriber system. It models a
 ...
 *       The contract maintains a registry of Providers and Subscribers, identified by their
 *       respective IDs. Each Provider has its list of Subscribers. A Subscribed should use a
 *       certain number of Providers. Additionally, a Provider can be in one of two states:
 *       active or inactive, depending on whether it can currently provide services or not. A
 *       subscription can be paused, so the Subscriber is not charged by Providers.
 */
contract ProviderController is Ownable {
  ...
  /// @dev max number of providers (200)
  uint64 public constant MAX_NUMBER_PROVIDERS = 200;
  /// @dev minimum fee (for example our minimum fee is 1000 gwei)
  uint256 public constant MIN_FEE = 1000 gwei;
  /// @dev fee calculation period (in this case, period is a month)
  uint256 public constant FEE_CALCULATION_PERIOD = 30 days;
  ...
  function registerProvider(
    ...
    require(providerId < MAX_NUMBER_PROVIDERS, "ProviderController: provider count reached max");
  ...
  function registerSubscriber(
    ...
    for (uint256 i = 0; i < _providerIds.length; i++) {
      ...
      totalFee += providers[_providerIds[i]].fee * 2;
    }
  ...
  function pauseSubscription(uint64 _subscriberId) external {
    ...
    for (uint64 i = 0; i < MAX_NUMBER_PROVIDERS; i++) {
      if (subscribeUpperIndex[_subscriberId][i] == 0) {
        continue;
      }

      // updates subscription balance
      uint256 index = subscribeUpperIndex[_subscriberId][i] - 1;
      (uint256 earning, ) = calculateSubscriptionEarning(i, _subscriberId);
      // cancel subscription
      cancelSubscribe(i, index);
      subscribeUpperIndex[_subscriberId][i] = 0;

      providers[i].balance += earning;
    }
  }
  ...
  function getSubscriberRemaining(uint64 _subscriberId) external view returns (int256 remaining) {
    ...
    for (uint64 i = 0; i < MAX_NUMBER_PROVIDERS; i++) {
      ...
      uint256 earning = getSubscriptionEarning(i, _subscriberId);
      totalUsed += earning;
    }
